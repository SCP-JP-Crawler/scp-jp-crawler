...
class WikiMixin:
    ...
    def parse_history(self, response, item, history_page=1):
        ...
        # Always keep a history key so downstream processing does not crash.
        item["history"] = changes
        try:
            history = response.json()
            if "body" not in history:
                self.logger.warning(f"History lookup returned no body for {item['url']}: {history}")
                return self.get_page_source_request(page_id, item)
            soup = BeautifulSoup(history["body"], "lxml")
            if not soup.table:
                self.logger.warning(f"History lookup returned no table for {item['url']}")
                return self.get_page_source_request(page_id, item)
    ...
    def follow_splash_redirects(self, response, tags, callback):
        ...
        if "adult" in tags:
            # Test Case - http://scp-jp.wikidot.com/scp-597
...
class ScpSpider(CrawlSpider, WikiMixin):
    ...
    # start_urls = [f"http://scp-jp.wikidot.com/scp-3318"]
